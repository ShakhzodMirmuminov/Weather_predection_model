...
if 'selected_city' not in st.session_state: 
    st.session_state.selected_city = 'Tashkent'
...
TASHKENT_LAT = UZBEKISTAN_CITIES["Tashkent"]["lat"]
TASHKENT_LON = UZBEKISTAN_CITIES["Tashkent"]["lon"]
...
def get_current_weather(city_name):
    ...
    lat = UZBEKISTAN_CITIES[city_name]["lat"]
    lon = UZBEKISTAN_CITIES[city_name]["lon"]
    ...
    try:
        ...
        st.error(f"Error fetching live weather for {city_name}: {e}")
        return None

@st.cache_data
def read_historical_data(city_name):
    """Load historical weather data from CSV for the specified city"""

    safe_city_name = city_name.replace(" ", "_").lower()
    filepath = os.path.join(DATA_DIR, f"{safe_city_name}_weather.csv")
    ...
    try:
        ...
        st.error(f"Historical data file not found for {city_name} at '{filepath}'. Please ensure you have run the data collection script and the data directory exists.")
        return None
...
def train_model_with_time_series_cv(historical_df, n_splits=5):
    ...
    exclude_cols = ['time', 'temperature_2m', 'precipitation', 'city']
...
def home_page():
    ...
    st.markdown("<p class='title-gradient'>Uzbekistan Weather Forecast ML</p>", unsafe_allow_html=True) # Updated Title
    st.markdown("<p class='subtitle'>Select a location in Uzbekistan to view the forecast powered by 23+ years of historical data.</p>", unsafe_allow_html=True)

    st.markdown("### Select Location")
    
    city_names = list(UZBEKISTAN_CITIES.keys())

    selected_city_index = city_names.index(st.session_state.selected_city) if st.session_state.selected_city in city_names else 0
    
    selected_city = st.selectbox(
        "Choose a city:",
        city_names,
        index=selected_city_index,
    ...
    st.markdown("### Map View (All Cities)")
    ...
    st.info(f"Currently selected city: **{selected_city}**. Click 'Open Forecast' to view the full prediction.")
...
def forecast_page(current_weather, historical_data, model_data):
    ...
    with st.spinner(f"⏳ Generating forecasts for {city_name}..."):
    ...
    st.markdown(f"## 📍 {city_name} Weather Forecast") # Updated Title
...
def main():
    ...
    if st.session_state.page == 'forecast':
        with st.spinner(f"🔄 Loading data and training models for {selected_city}..."):

            current_weather = get_current_weather(selected_city)
            historical_data = read_historical_data(selected_city)
    ...
    if st.session_state.page == 'home':
        ...
        # NOTE: model_data is only available if we were on the forecast page
        # If the user goes from home to forecast, the model_data will be computed above.
        # If a file error occurs, st.stop() prevents reaching this point.
